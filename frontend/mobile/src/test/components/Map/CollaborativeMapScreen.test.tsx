...
// Mock para el componente CollaborativeMapScreen
jest.mock('../../../components/Map/CollaborativeMapScreen', () => {
  const React = require('react');
  const { View, Text, TouchableOpacity } = require('react-native');
  
  return function MockCollaborativeMapScreen({ mapId, userId }: CollaborativeMapScreenProps) {
    const handleAddPOI = () => {
    ...
    return React.createElement(View, { testID: 'collab-map-screen' }, [
...
import CollaborativeMapScreen from '../../../components/Map/CollaborativeMapScreen';
import * as mapService from '../../../services/collaborativeMap.service';

describe('CollaborativeMapScreen', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  
  test('renderiza correctamente', () => {
    const { getByTestId } = render(<CollaborativeMapScreen mapId="map1" userId="user1" />);
    expect(getByTestId('collab-map-screen')).toBeTruthy();
  ...
  test('carga los datos del mapa al inicializar', () => {
    render(<CollaborativeMapScreen mapId="map1" userId="user1" />);
